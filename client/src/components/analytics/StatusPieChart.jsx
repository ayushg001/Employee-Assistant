...
import { STATUS_DATA as FALLBACK_DATA } from '../../data/mockAnalytics';
...
export default function StatusPieChart({ data }) {
  ...
  const activeItem = chartData.find((d) => d.name === 'Active');
  const activePct = total > 0 && activeItem ? Math.round((activeItem.value / total) * 100) : 100;
          ...
          <PieChart>
            ...
            >
              {chartData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={entry.color || '#10b981'} stroke="transparent" />
              ))}
            </Pie>
            <Legend
